...
#看起来就就类似fstream，把filename指针下的size大小的文件读进来，然后把f这个fstream的内容每4个字节来读,读完解释成float，然后挂到z后面
...
def RunConv(conv,Kx,Ky,Sx,Sy,mode,relu_en,feature_in,W,bias,feature_out):
    #conv.write是pynq自带的函数，conv就理解成函数指针好了，就理解成python的函数定义方法是 def 名称(函数指针名字，参数...)，这里利用了python读输入图像很方便的cv2库，来省去了专门在参数列表写明："输入通道 高 宽"的事情，直接用feature_in.shape完成了
    conv.write(0x10,feature_in.shape[2]);#在python里面自带的.shape功能，shape[2]就是层数，就是输入通道数CHin
    conv.write(0x18,feature_in.shape[0]);#输入图层的高度Hin
    conv.write(0x20,feature_in.shape[1]);#输入图层的宽度Win
    ...
    conv.write(0, (conv.read(0)&0x80)|0x01 );#这里就是参照Xpool.c里面的XPool_Start来照做的
    tp=conv.read(0)
    while not ((tp>>1)&0x1):#这里在做IsDone的判断，没完成就在这停顿
        tp=conv.read(0);
    #print(tp);
...
#上面一大通就是赋了一大堆初值，总的来说我们做了2次卷积和池化，然后全连接

xlnk=Xlnk();

ol=Overlay("test0829.bit")
ol.ip_dict
ol.download()
conv=ol.Conv_0
pool=ol.Pool_0
...
#开辟空间；cma_array是xlnk包下，开辟多维数组空间的函数
#input image
...
#Initialize W, bias
#W 权重的初始化，从bin读出来，然后赔给自己在前面声明的W多维数组
w_conv1=readbinfile("/home/xilinx/data/W_conv1.bin",KERNEL_HEIGHT1*KERNEL_WIDTH1*IN_CH1*OUT_CH1)
...
#初始化Bias
B_conv1=readbinfile("/home/xilinx/data/b_conv1.bin",OUT_CH1)
for i in range(OUT_CH1):
	b_conv1[i]=B_conv1[i]

w_conv2=readbinfile("/home/xilinx/data/W_conv2.bin",KERNEL_HEIGHT2*KERNEL_WIDTH2*IN_CH2*OUT_CH2)
...
B_conv2=readbinfile("/home/xilinx/data/b_conv2.bin",OUT_CH2)
for i in range(OUT_CH2):
	b_conv2[i]=B_conv2[i]

#初始化全连接的权重和偏置
w_fc1=readbinfile("/home/xilinx/data/W_fc1.bin",KERNEL_HEIGHT3*KERNEL_WIDTH3*IN_CH3*OUT_CH3)
...
B_fc1=readbinfile("/home/xilinx/data/b_fc1.bin",OUT_CH3)
for i in range(OUT_CH3):
	b_fc1[i]=B_fc1[i]

w_fc2=readbinfile("/home/xilinx/data/W_fc2.bin",KERNEL_HEIGHT4*KERNEL_WIDTH4*IN_CH4*OUT_CH4)
...
B_fc2=readbinfile("/home/xilinx/data/b_fc2.bin",OUT_CH4)
...
while(1):
	while(1):
		...
		break
	image1=cv2.imread("/home/xilinx/data/1.jpg",cv2.IMREAD_GRAYSCALE).astype(np.float32)
	print("Read image")
	...
	#conv1
	RunConv(conv,KERNEL_WIDTH1,KERNEL_HEIGHT1,X_STRIDE1,Y_STRIDE1,MODE1,RELU_EN1,image,W_conv1,b_conv1,h_conv1)#注意 这里的h_conv1是输出结果,把结果给下面的RunPool了，后面都一样，这样做到了参数传递
